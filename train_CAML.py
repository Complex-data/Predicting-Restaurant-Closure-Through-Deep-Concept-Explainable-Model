...
class CFExperiment(Experiment):
    ...
    def _prepare_set(self, strmode, k, data):
        ...
        if strmode=='111':
            item_reviews_encode_filename = str(k)+'_item_reviews_encode.txt'
            item_concepts_encode_filename = str(k)+'_item_concepts_encode.txt'
            user_reviews_encode_filename = str(k)+'_user_reviews_encode.txt'
            user_concepts_encode_filename = str(k)+'_user_concepts_encode.txt'
            item_reviews_encode_file = open(item_reviews_encode_filename,'w+')
            item_concepts_encode_file = open(item_concepts_encode_filename,'w+')
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.epochs+1):
            ...
            for i in tqdm(range(0, num_batches+1)):
                ...
                if(len(batch)==0):
                    continue
                batch = self._prepare_set('train',i,batch)	#准备caml模型的需要用到的数据，包括各个输入向量
